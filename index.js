...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN: Sands BOT\n'//GANTI NAMA LU COK
            + 'ORG: Sands BOT;\n'//GANTI NAMA LU!!
            + 'TEL;type=CELL;type=VOICE;waid=559499553950:559499553950\n'//GANTI NOMOR LU
            + 'END:VCARD'
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const FadhilGraphy = ["559499553950@s.whatsapp.net"]
			...
			mess = {
				...
				only: {
					...
					ownerB: '*[⚠️]Desculpe, este comando só pode ser usado pelo Sands!* ',
					premium: '*[⚠️]Desculpe, este comando só pode ser usado por usuários Premium!!*',
					userB: `[ ⚠️NÃO REGISTRADO⚠]\n\n️ SALVE *${pushname2}* Você ainda não esta registrado NO [Sands-BOT] \nRegistre-se primeiro Para ter acesso ao menu\n\exemplo: ${prefix}daftar Sands/21`,
			...
			const ownerNumber = [
			"9499553950@s.whatsapp.net"//GANTI NOMOR LU
			]
			premium = [
			"5594553950@s.whatsapp.net","5515998490542@s.whatsapp.net","5511948131247@s.whatsapp.net","558592569721@s.whatsapp.net","558499781544@s.whatsapp.net","555196812531@s.whatsapp.net","558293874218@s.whatsapp.net","553888118651@s.whatsapp.net","556999263728@s.whatsapp.net","558181822045@s.whatsapp.net","556596305054@s.whatsapp.net","556593559865@s.whatsapp.net"]//GANTI NOMOR YAG MAU DI PREM, GABISA NYIMPEN DI DATABASE!!
			]
        ...
        const limitAdd = (sender) => {
            ...
            if (args.match(/(https:\/\/chat.whatsapp.com)/gi)) {
                ...
                if (!check) {
                    ...
                    reply('*[LINK DE GRUPO DETECTADO!]*\ NÃO VOU TE REMOVER DO GRUPO SEU FDP DA PRÓXIMA VEZ PRESTE ATENÇÃO NAS REGRAS.').then(() => {
                        client.groupRemove()
                    })
                }
            }
...
/*********END PISAH MENU***********/
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           client.sendMessage(from, options, text, { quoted: mek } )
				...
				case 'leave': 
	                ...
	                break
	            case 'getses':
                    if (!isOwner) return client.reply(from, 'Este comando é apenas para o Sands', id)
         ...
         case 'lirik':
                ...
                case 'report':
                    ...
                    reply('Problemas foram relatados ao Sands, relatórios falsos não serão respondidos..')
                    break
              case 'request':
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('9499553950@s.whatsapp.net', options, text, {quoted: mek})
                    reply('As solicitações foram relatadas ao Sands, as solicitações que podem sobrecarregar o proprietário não serão respondidas.')
                    break
				case 'meme':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
	   ...
	   case 'coli':
		    ...
		    case 'map':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 Sands BOT 〙', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				case 'clearall':
					...
					reply(`\`\`\`deleta todo o chat do Sands-BOT\`\`\``)
					break
                                case 'bcgc':
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: '✔️ FIGURINHA CONVERTIDA'✔️})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender) 
					break 
				...
				default:
                  ...
                  reply(`Desculpe man *${pushname2}*, Comando *${prefix}${command}* nn registrado no menu! pfv chame o bot novamente *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[Sands-BOT]','Yellow'), 'Comando não registrado', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'yellow'))
